		...
		<div class="card">
			<div class="card-header custom_cart_header">
				<ul class="dashboard_control float-right" >
					...
					<li>
						<form action="{{ route('downloadFile') }}" method="post">@csrf
							<input type="hidden" id="file_id" name="file_id" />
							<button type="button" id="download_btn" value="{{ $whchjobs }}" class="action-button" data-toggle="tooltip" data-placement="top" title="Download"><i class="fa fa-download" aria-hidden="true"></i>
							</button>
						</form>
					</li>
					<li>
						<form action="{{ route('downloadFile') }}" method="post">@csrf
							<input type="hidden" id="file_id" name="file_id" />
							<button type="button" id="exportzip_btn" value="{{ $whchjobs }}" class="action-button" data-toggle="tooltip" data-placement="top" title="Export zip"><i class="fas fa-file-archive" aria-hidden="true"></i>
							</button>
						</form> <a href="{{ url('public/export.zip') }}" download id="download_zip" hidden>Download zip</a>
					</li>
					<li>
...
<div id="file_modal" class="modal fade" role="dialog">
	<div class="modal-dialog">
		...
		<div class="modal-content">
			<form id="add_file" method="POST" enctype="multipart/form-data">@csrf
				...
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="col-md-9">
							<select name="jobs[]" class="form-control" multiple style="margin:0;">@if($jobs) @foreach($jobs as $jobkey => $job)
								<option value="{{ $job->job_id }}">{{$job->job_name}}</option>@endforeach @endif</select>
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-sm-12">
						<div class="form-group">
							...
							<select id="select_job_dash" class="form-control">@foreach($jobs as $jkey => $job)
								<option <?php if($job->job_id==$whchjobs) { ?> selected
									<?php } ?>value="{{ $job->job_id }}">{{ $job->job_name }}</option>@endforeach</select>
						</div>
					</div>
				</div>
			</div>
