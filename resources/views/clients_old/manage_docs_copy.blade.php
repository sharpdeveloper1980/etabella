...
img.file_iocn {
   ...
   <div class="row second_header">
	  ...
	  <button style="border: 2px solid #e54e09 !important;width: 88px;height: 28px;background: white;border-radius: 5px;color:#e54e09;margin-left: 20px;" onclick="goBack()" data-toggle="tooltip" title="Go Back">Go Back</button>
				...
				<ul class="list-group" id="sortable" >
					...
					@foreach($all_witness_file as $file)
						...
						<li  class="list-group-item"  id="{{ $file['id'] }}" data-id="{{ $file['id'] }}" >
							<a href="{{ asset('clients/examination_schedule_render/'.$file['doc_id']).'/'.Request::segment(3) }}"><div style="width:95%;float:left"><span >{{ $i }}</span> &nbsp; <span1 style="background:{{ $color }};width: 46px;padding-left: 12px;border-radius: 50%;">&nbsp;</span1> &nbsp; <img style="width:20px" class="file_iocn" src="{{ asset('public/images/pdf.ico') }}"> {{ $file['file_name'] }}</div></a>
...
function checkfile(type)
{
  ...
  $("#add_files").click(function(){
   ...
   function refreshPage(whchjobs){
	...
	function goBack() {
	  window.location.href="{{ url('clients/examination_schedule_inner/'.Request::segment(4)) }}";
	}
</script>
@stop
@endsection
