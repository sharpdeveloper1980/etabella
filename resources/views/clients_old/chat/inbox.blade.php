          ...
          $(".jspContainer").mCustomScrollbar({
            ...
            callbacks:{
              onTotalScrollBack:function(){
                ...
                $.ajax({
                    type: "GET",
                    url: baseurl+"/clients/get_previous_messages/<?php  if($selected_grp) { echo $selected_grp->group_id; } ?>/"+last_id,
                    dataType: 'json',
...
/* file uploader section */
  ...
  if (file) {
    ...
    function get_messages(percentage){ 
        ...
        $.ajax({
            type: "GET",
            url: baseurl+"/clients/get_group_messages/<?php if($selected_grp) { echo $selected_grp->group_id; } ?>/"+last_msgs,
            dataType: 'json',
            success: function(data)
            {
              // console.log(data);
  ...
  return strTime;
    ...
    if(trim_msg.length > 0 && grp_id){
      ...
      $.ajax({
        type: "POST",
        url: baseurl+"/clients/send_msg_group",
        data: {_token: "{{ csrf_token() }}",message:msg,group_id:grp_id,sender_id:sender_id,msg_type:"msg",job:whchjobs},
        dataType: 'text',
        success: function(data)
        {
          console.log(data);
